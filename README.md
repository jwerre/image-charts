...
Authenticate your account when using the API by including your account id and secret API key. Be sure to keep them secret! To use your API key, pass it to the ImageCharts module. The library will then automatically send this key with each request. There are multiple ways to pass your api key to the ImageCharts Node.js module:
...
If you don't pass your API key directly to the module it will automatically look for it in a '.image_charts.json' file in your home directory. Be sure that the file is valid JSON and has the value `account` and `secret`. Specifying an 

```bash
'{"account": "<your account id>", "secret": "<your api key>"}' > ~/.image_charts.json
```

### Create a config file environmental variable

If you'd like to store your API key in a location other than your home folder, create the environmental variable `IMAGE_CHARTS_AUTH` that points to a valid json file.
...
```bash
export IMAGE_CHARTS_ID=/path/to/my/my_image_charts_auth.json
export IMAGE_CHARTS_SECRET=/path/to/my/my_image_charts_auth.json
```
...
(async function(){
	...
	try {
		pieChart = await imageCharts.pie(data);
	} catch (err) {
...
Set the timeout for requestions. Default: Node.js default (require('http').createServer().timeout; => 20000)
...
```js
imageChars.version('<api-version>')
```
#### Secret
